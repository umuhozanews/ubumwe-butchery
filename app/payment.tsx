...
const WA_NUMBER   = '250798989741';

export default function PaymentScreen() {
  const params = useLocalSearchParams<{
    orderId: string; name: string; phone: string;
    total: string; itemCount: string;
  }>();
  ...
  const itemCount     = Array.isArray(params.itemCount)  ? params.itemCount[0]  : (params.itemCount  ?? '1');
  ...
  function dialCode() {
    ...
    Linking.openURL(`tel:${encoded}`).catch(() =>
      Alert.alert('Sibyo', 'Tegeka uri ku muvugishirize.')
    );
  }

  function openWhatsApp() {
    const msg = `Muraho UBUMWE BUTCHERY!\n\nNshaka inkunga ku itumba ryanjye:\n• Izina: ${customerName}\n• Telefone: ${customerPhone}\n• Itumba: #${orderId.slice(0, 8).toUpperCase()}\n\nMurakoze!`;
  ...
  async function pickFromGallery() {
    ...
    if (status !== 'granted') {
      Alert.alert('Uruhushya', 'Dukenewe uruhushya rwo kugera ku mashusho yawe.');
      return;
    }
  ...
  async function takePhoto() {
    ...
    if (status !== 'granted') {
      Alert.alert('Uruhushya', 'Dukenewe uruhushya rwo gukoresha kamera yawe.');
      return;
    }
  ...
  async function uploadProof(uri: string): Promise<string | null> {
    try {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      Alert.alert('Ikosa', 'Ikibazo cyabaye mu gutanga icyemezo. Ongera ugerageze.');
    } finally {
      setUploading(false);
    }
  }
      ...
      <View style={styles.content}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.summaryCard}>
            ...
            {[
              ['Inomero', `#${orderId.slice(0, 8).toUpperCase()}`],
              ['Umuguzi',  customerName],
              ['Telefone', customerPhone],
              ['Ibicuruzwa', `${itemCount} ${parseInt(itemCount) === 1 ? 'igicuruzwa' : 'ibicuruzwa'}`],
              ...(total ? [['Igiciro Cyose', `${parseInt(total).toLocaleString()} RWF`]] : []),
            ].map(([label, value], i, arr) => (
              <View key={label} style={[styles.summaryRow, i === arr.length - 1 && { borderBottomWidth: 0 }]}>
                <Text style={styles.summaryLabel}>{label}</Text>
                <Text style={[styles.summaryValue, label === 'Igiciro Cyose' && { color: colors.primary, fontFamily: fonts.bold }]}>
                  {value}
                </Text>
              </View>
            ))}
          ...
          <View style={styles.momoCard}>
            ...
            <View style={styles.stepsBox}>
              {[
                ...
                { n: '2', t: 'Ishyura ukoresheje code iri hasi' },
                { n: '3', t: 'Nyuma yo kwishyura, fata ifoto ya message ya MoMo' },
                { n: '4', t: 'Shyiraho ifoto y\'icyemezo, ukande EMEZA' },
              ].map((s) => (
          ...
          <Pressable style={styles.waBtn} onPress={openWhatsApp}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.waBtnTitle}>Ufite ikibazo? Tuvugane</Text>
        ...
        <SafeAreaView style={styles.confirmSafe} edges={['bottom']}>
          <Pressable
            style={[styles.confirmBtn, (!proofUri || uploading) && styles.confirmDisabled]}
            onPress={handleConfirm}
            disabled={!proofUri || uploading}
          >
            {uploading ? (
            ...
            <Text style={styles.confirmText}>
              {uploading ? 'Gutegereza...' : 'Emeza Itumba'}
            </Text>
          </Pressable>
        </SafeAreaView>
      </View>
...
const styles = StyleSheet.create({
  ...
  momoCard: {
    backgroundColor: colors.headerBg, borderRadius: radius.card, padding: 18,
  ...
  dialBtn: {
    ...
    backgroundColor: colors.primary, borderRadius: radius.button, paddingVertical: 12, gap: 6,
  },
  dialBtnText: { fontFamily: fonts.semiBold, fontSize: 14, color: '#fff' },
  copyBtn: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    backgroundColor: 'rgba(255,255,255,0.12)', borderRadius: radius.button, paddingVertical: 12, gap: 6,
  ...
  uploadBtn: {
    ...
    backgroundColor: '#f0fdf4', borderRadius: radius.button, paddingVertical: 24, gap: 8,
  ...
  waBtn: {
    ...
    backgroundColor: '#25D366', borderRadius: radius.card, padding: 16,
    shadowColor: '#25D366', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 8, elevation: 6,
  },
  ...
  confirmSafe: {
    backgroundColor: colors.contentBg, paddingHorizontal: 16, paddingTop: 10,
  ...
  homeBtn: {
    width: '100%', borderRadius: radius.button, paddingVertical: 14, alignItems: 'center',
